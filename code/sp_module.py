...
class SolarPilot:
    ...
    def plot_flux_map(self, flux, name = None, flip_z_axis = False):
        ...
        if flip_z_axis: #use for SolarPILOT imports - this is done automatically in HALOS
            flux = flux[::-1]
...
class SP_Field(SolarPilot):
    ...
    def generate_field(self, not_filter_helio = False, plotting = False):
        ...
        solar_field.to_csv("./../outputs/SolarPILOT_field.csv",index=False)
        if plotting is True:
