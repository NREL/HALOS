# -*- coding: utf-8 -*-
"""
Created on Thu Oct  8 13:32:15 2020
...
class SolarPilot:
    ...
    def assign_inputs(self, weather_data = None, hour_id = None, dni = None, 
        ...
        ##Field Parameters:
        cp.data_set_number(self.r, b"solarfield.0.dni_des", float(self.receiver_data["field_dni"]))   
        cp.data_set_number(self.r, b"solarfield.0.q_des", ((float(self.receiver_data["power_rating"]))/10**6)) 
        cp.data_set_number(self.r, b"solarfield.0.tht", float(self.receiver_data["tow_height"]))  
        ##Mirror Parameters - (Heliostats) 
        cp.data_set_number(self.r, b"heliostat.0.height", float(self.receiver_data["mirror_ht"]))
        cp.data_set_number(self.r, b"heliostat.0.width", float(self.receiver_data["mirror_len"]))
        ##Receiver Parameters:
        ...
        cp.data_set_number(self.r, b"receiver.0.rec_height", float(self.receiver_data["height"]))  
        cp.data_set_number(self.r, b"receiver.0.rec_azimuth", 180 - float(self.receiver_data["azimuth_deg"]))   
        cp.data_set_number(self.r, b"receiver.0.rec_elevation",90 - float(self.receiver_data["zenith_deg"]))    
        cp.data_set_number(self.r, b"receiver.0.rec_offset_x",float(self.receiver_data["rec_cent_offset_x"]))   
        cp.data_set_number(self.r, b"receiver.0.rec_offset_y",float(self.receiver_data["rec_cent_offset_y"]))   
        cp.data_set_number(self.r, b"receiver.0.rec_offset_z",float(self.receiver_data["rec_cent_offset_z"])) 
        if self.receiver_data["receiver_type"] == 'Flat plate':
            cp.data_set_number(self.r, b"receiver.0.rec_width", float(self.receiver_data["length"]))   
        if self.receiver_data["receiver_type"] == 'External cylindrical':
            cp.data_set_number(self.r, b"receiver.0.rec_diameter", float(self.receiver_data["diameter"]))
        #Receiver Type: External cylindrical or Flat plate
        rec_type = bytes(self.receiver_data["receiver_type"], encoding="utf-8")
        cp.data_set_string(self.r, b"receiver.0.rec_type",rec_type) 
        ##Flux Parameters
        if dni is not None:
            cp.data_set_number(self.r, b"fluxsim.0.flux_dni", dni)
        else: 
            cp.data_set_number(self.r, b"fluxsim.0.flux_dni", float(self.receiver_data["flux_dni"]))
        cp.data_set_number(self.r, b'fluxsim.0.x_res', float(self.receiver_data["pts_per_dim"]))   
        cp.data_set_number(self.r, b'fluxsim.0.y_res', float(self.receiver_data["pts_per_dim"]))  
        if hour_id is not None:
            cp.data_set_number(self.r, b"fluxsim.0.flux_day", weather_data['day'][hour_id])
            cp.data_set_number(self.r, b"fluxsim.0.flux_hour", weather_data['hour'][hour_id])
            cp.data_set_number(self.r, b"fluxsim.0.flux_month", weather_data['month'][hour_id])
            cp.data_set_number(self.r, b"fluxsim.0.flux_dni", weather_data['dni'][hour_id])
        #Weather File 
        w_file = bytes(self.filenames["weather_filename"], encoding="utf-8")
        cp.data_set_string(self.r, b"ambient.0.weather_file", w_file) 
        if not_filter_helio is True:
            cp.data_set_string( self.r, b"solarfield.0.des_sim_detail", b"Do not filter heliostats" )
        if center_aimpoint is True:
            cp.data_set_string(self.r, b"fluxsim.0.aim_method", b"Simple aim points")
...
class SP_Flux(SolarPilot):
    ...
    def get_single_helio_flux_dict(self,aim_method = 'Simple aim points', weather_data = None, hour_id = None, dni = None):
        self.assign_inputs(weather_data, hour_id, dni)
        cp.data_set_string(self.r, b"fluxsim.0.aim_method", bytes(aim_method, encoding="utf-8"))
        cp.data_set_string(self.r, b"solarfield.0.des_sim_detail", b"Do not filter heliostats")
    ...
    def sp_aimpoint_defocus(self, weather_data = None, hour_id = None, dni = None):
        ...
        #comb_flux, defocused_helios = flux_limit_heuristic(flux_dict, 200.)
