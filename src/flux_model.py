...
class FluxModel(object):
    ...
    def ShiftImage_GenerateSingleHeliostatFluxMaps(self,helio_idx,solar_vector,approx=True):
        ...
        if self.receiver.params["receiver_type"] == "External cylindrical":
            ...
            # def shift_center_map():
            #     #Shifting Center Map
            #     import math
            #     angle_rad = math.atan2(self.field.y[helio_idx], self.field.x[helio_idx])
            #     angle_deg = (180/numpy.pi)*angle_rad   ##Zero at X (+ive or East) Positive towards North (+y) and negative towards(South)
            #     section_field = 6    ## 360/6 = 60
            #     center_shift = (self.receiver.params["pts_per_dim"]//section_field) + (0 if self.receiver.params["pts_per_dim"] % section_field == 0 else 1)
            #     #center_shift = (0 if self.receiver.params["pts_per_dim"] % section_field == 0 else 1)
            #     map_center = self.GenerateSingleFluxMap(helio_idx,center_idx,solar_vector,dni,approx)
            #     if angle_deg >= 60 and angle_deg <= 120:
            #         map_center = map_center
            #     elif angle_deg < 60 and angle_deg >= 0:
            #         map_center = shift_2d_array(map_center, -(center_shift), 0, constant = 0)
            #     elif angle_deg > 120: 
            #         map_center = shift_2d_array(map_center, center_shift, 0, constant = 0)
            #     elif (angle_deg >-60 and angle_deg < 0):
            #         map_center = shift_2d_array(map_center, -2 * (center_shift), 0, constant = 0)
            #     elif angle_deg <= (-60) and angle_deg >= (-120):
            #         map_center = shift_2d_array(map_center, (3*center_shift), 0, constant = 0)
            #     elif angle_deg <= -(120):
            #         map_center = shift_2d_array(map_center, 2 * center_shift, 0, constant = 0)
            #     return map_center
            # map_center = shift_center_map()
